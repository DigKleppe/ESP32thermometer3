...
static esp_err_t http_resp_dir_html(httpd_req_t *req, const char *dirpath)
{
	...
	httpd_resp_sendstr_chunk(req, "<table class=\"fixed\" border=\"1\">"
								  "<col width=\"800px\" /><col width=\"300px\" /><col "
								  "width=\"300px\" /><col width=\"100px\" />"
...
static esp_err_t download_get_handler(httpd_req_t *req)
{
	...
	if (stat(filepath, &file_stat) == -1)
	{
		...
		if (g_pCGIs)
		{
			...
			{
				if (strncmp(filename, g_pCGIs->pcCGIName, strlen(filename)) == 0)
				{
					 ...
					 */
					filename = g_pCGIs->pfnCGIHandler(i, params);
					foundCGI = true;
					break;
        }
          else
		...
		if (stat(filename, &file_stat) == -1)
		{
			...
			do
			{
				...
				if (chunksize > 0)
				{
					...
					{
						//	if (httpd_send_all( req,(const char *) chunk,(size_t) chunksize)
						//!= ESP_OK) {
	...
	//	ESP_LOGI(TAG, "File sending complete %s (%ld bytes)", filename,
	// file_stat.st_size);
...
static esp_err_t upload_post_handler(httpd_req_t *req)
{
	...
	//		httpd_resp_send_err(req, HTTPD_500_INTERNAL_SERVER_ERROR, "Invalid
	// filename"); 		return ESP_FAIL;
	//	}

	//	if (stat(filepath, &file_stat) == 0) {
	//		ESP_LOGE(TAG, "File already exists : %s", filepath);
	//		/* Respond with 400 Bad Request */
	//		httpd_resp_send_err(req, HTTPD_400_BAD_REQUEST, "File already
	// exists"); 		return ESP_FAIL;
	//	}
...
static esp_err_t put_handler(httpd_req_t *req)
{
	...
	//	/* Skip leading "/upload" from URI to get filename */
	//	/* Note sizeof() counts NULL termination hence the -1 */
	//	const char *filename = get_path_from_uri(filepath, ((struct
	// file_server_data *)req->user_ctx)->base_path, 			req->uri + sizeof("/upload") -
	// 1, sizeof(filepath)); 	if (!filename) {
	//		/* Respond with 500 Internal Server Error */
	//		httpd_resp_send_err(req, HTTPD_500_INTERNAL_SERVER_ERROR, "Filename
	// too long"); 		return ESP_FAIL;
	//	}
	//
	//	/* Filename cannot have a trailing '/' */
	//	if (filename[strlen(filename) - 1] == '/') {
	...
	while (remaining > 0)
	{
		...
		//			ESP_LOGE(TAG, "File write failed!");
		//			/* Respond with 500 Internal Server Error */
		//			httpd_resp_send_err(req, HTTPD_500_INTERNAL_SERVER_ERROR, "Failed to
		// write file to storage"); 			return ESP_FAIL;
		//		}
		//
		/* Keep track of remaining size of
		 * the file left to be uploaded */
