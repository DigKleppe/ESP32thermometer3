...
static void setStaticIp(esp_netif_t *netif) {
  ...
  //  ipaddr_addr(EXAMPLE_MAIN_DNS_SERVER), ESP_NETIF_DNS_MAIN));
  //  ESP_ERROR_CHECK(example_set_dns_server(netif,
...
static void smartconfigTask(void *parm) {
  ...
  while (1) {
    uxBits = xEventGroupWaitBits(
        s_wifi_event_group, CONNECTED_BIT | ESPTOUCH_DONE_BIT, true, false,
        (SMARTCONFIGTIMEOUT * 1000) / portTICK_PERIOD_MS);
...
static void event_handler(void *arg, esp_event_base_t event_base,
  ...
  if (event_base == WIFI_EVENT && event_id == WIFI_EVENT_STA_START) {
    ...
    if (s_retry_num < EXAMPLE_ESP_MAXIMUM_RETRY) {
      ...
      xTaskCreate(smartconfigTask, "smartconfig_example_task", 4096, NULL, 3,
                  NULL);
      s_retry_num = 0;
...
void wifiConnect(void) { wifi_init_sta(); }
...
// wifiSettings_t wifiSettings;
// wifiSettings_t wifiSettingsDefaults = { CONFIG_EXAMPLE_WIFI_SSID,
// CONFIG_EXAMPLE_WIFI_PASSWORD,false  };
//
///* The examples use WiFi configuration that you can set via project
///configuration menu
//
...
//        ESPTOUCH_DONE_BIT, true, false, (SMARTCONFIGTIMEOUT * 1000) /
//        portTICK_PERIOD_MS); if(uxBits & CONNECTED_BIT) {
//            ESP_LOGI(TAG, "WiFi Connected to ap");
//        }
//        if(uxBits & ESPTOUCH_DONE_BIT) {
//            ESP_LOGI(TAG, "smartconfig over");
...
// event_id, void *event_data) { 	if (event_base == WIFI_EVENT && event_id ==
//WIFI_EVENT_STA_START) { 		esp_wifi_connect(); 	} else if (event_base ==
//WIFI_EVENT && event_id == WIFI_EVENT_STA_DISCONNECTED) { 		if (s_retry_num <
//EXAMPLE_ESP_MAXIMUM_RETRY) { 			esp_wifi_connect(); 			connectStatus = CONNECTING;
//			s_retry_num++;
//			ESP_LOGI(TAG, "retry to connect to the AP");
//		} else {
//			xTaskCreate(smartconfigTask, "smartconfig_example_task",
//4096, NULL, 3, NULL); 			s_retry_num = 0; 			connectStatus = SMARTCONFIG_ACTIVE;
//			ESP_LOGI(TAG, "Starting SmartConfig");
//		}
//		xEventGroupClearBits(s_wifi_event_group, CONNECTED_BIT);
//	} else if (event_base == IP_EVENT && event_id == IP_EVENT_STA_GOT_IP) {
...
//		ESP_LOGI(TAG, "Scan done");
//	} else if (event_base == SC_EVENT && event_id == SC_EVENT_FOUND_CHANNEL)
//{ 		ESP_LOGI(TAG, "Found channel"); 	} else if (event_base == SC_EVENT &&
//event_id == SC_EVENT_GOT_SSID_PSWD) { 		ESP_LOGI(TAG, "Got SSID and password");
//		connectStatus = CONNECTED;
//		smartconfig_event_got_ssid_pswd_t *evt =
//(smartconfig_event_got_ssid_pswd_t*) event_data; 		wifi_config_t wifi_config;
//		uint8_t ssid[33] = { 0 };
//		uint8_t password[65] = { 0 };
//		uint8_t rvd_data[33] = { 0 };
//
...
//sizeof(wifi_config.sta.ssid)); 		memcpy(wifi_config.sta.password, evt->password,
//sizeof(wifi_config.sta.password)); 		wifi_config.sta.bssid_set = evt->bssid_set;
//		if (wifi_config.sta.bssid_set == true) {
...
//		ESP_LOGI(TAG, "PASSWORD:%s", password);
//
//		memcpy ( (char *)wifiSettings.SSID, ssid, sizeof (
//wifiSettings.SSID)); 		memcpy ( (char *)wifiSettings.pwd, password,sizeof (
//wifiSettings.pwd)); 		saveSettings();
//
//		if (evt->type == SC_TYPE_ESPTOUCH_V2) {
//			ESP_ERROR_CHECK(esp_smartconfig_get_rvd_data(rvd_data,
//sizeof(rvd_data))); 			ESP_LOGI(TAG, "RVD_DATA:"); 			for (int i = 0; i < 33; i++) {
//				printf("%02x ", rvd_data[i]);
//			}
//			printf("\n");
//		}
...
////	xTaskCreate(wifiConnectTask, "wifiConnectTask", 2* 4096, NULL, 3,
///handle);
//	wifi_init_sta();
//}
//
